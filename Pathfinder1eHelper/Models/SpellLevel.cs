...
public sealed class SpellLevel
{
    ...
    [Column(Name = "class_name", IsPrimary = true)] public string ClassName { get; set; } = string.Empty;

    [Column(Name = "level")] public int Level { get; set; }

    /// <summary><c>class</c>（主职业）或 <c>domain</c>（领域/子域）。</summary>
    [Column(Name = "kind", IsPrimary = true)] public string Kind { get; set; } = string.Empty;
}
